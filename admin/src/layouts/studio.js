...
const MenuLoop = (authorities, role, menulist) => {
	...
	return menulist.filter((item) => !!item.name && !!item.path).map((item) => {
...
const StudioMenu = ({ authorities, role, items, collapsed, mode, theme }) => (
	<Menu theme={theme || 'dark'} mode={mode || 'inline'} inlineCollapsed={mode !== 'horizontal' && collapsed}>
		...
		return (
			<DocumentTitle title={config.TITLE}>
				<Layout>
					...
					<Layout
						className={`${menuLayout === 'sidemenu' && styles.mainLayout} ${collapsed &&
						menuLayout === 'sidemenu'
