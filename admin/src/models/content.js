...
const API_URL = config.apiRoot + '/content';
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put, select }) {
      ...
      const { queryParams } = yield select((state) => state.content);
    ...
    *save({ payload }, { call, put, select }) {
      const { selectedNode } = yield select((state) => state.content);
    ...
    *remove({ payload }, { call, select }) {
      const selectedRows = yield select((state) => state.content.selectedRows);
