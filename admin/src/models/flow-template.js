...
const API_URL = config.apiRoot + '/flow/template';
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put, select }) {
      ...
      const { queryParams } = yield select((state) => state.flowTemplate);
    ...
    *save({ payload }, { call, put, select }) {
      const { selectedNode } = yield select((state) => state.flowTemplate);
    ...
    *remove({ payload }, { call, select }) {
      const selectedRows = yield select((state) => state.flowTemplate.selectedRows);
    ...
    *export({ payload }, { call, select }) {
      ...
      const { queryParams, columns, fields } = yield select((state) => state.flowTemplate);
