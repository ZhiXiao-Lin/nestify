...
const API_URL = config.apiRoot + '/service';
...
export default {
  ...
  effects: {
    ...
    *fetch({ payload }, { call, put, select }) {
      ...
      const { queryParams } = yield select((state) => state.service);
    ...
    *save({ payload }, { call, put, select }) {
      const { selectedNode } = yield select((state) => state.service);
    ...
    *remove({ payload }, { call, select }) {
      const selectedRows = yield select((state) => state.service.selectedRows);
    ...
    *export({ payload }, { call, select }) {
      ...
      const { queryParams, columns, fields } = yield select((state) => state.service);
