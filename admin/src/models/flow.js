...
const API_URL = config.apiRoot + '/flow';
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put, select }) {
      ...
      const { queryParams } = yield select((state) => state.flow);
    ...
    *save({ payload }, { call, put, select }) {
      const { selectedNode } = yield select((state) => state.flow);
    ...
    *dispatch({ payload }, { call }) {
      const { callback } = payload;
      !!callback && delete payload.callback;

      const res = yield call(apiPost, API_URL + '/dispatch', payload);
    ...
    *export({ payload }, { call, select }) {
      ...
      const { queryParams, columns, fields } = yield select((state) => state.flow);
