...
const API_URL = config.apiRoot + '/carousel';
...
export default {
  ...
  effects: {
    *fetch({ payload }, { call, put, select }) {
      ...
      const { queryParams } = yield select((state) => state.carousel);
    ...
    *save({ payload }, { call, put, select }) {
      const { selectedNode } = yield select((state) => state.carousel);
    ...
    *remove({ payload }, { call, select }) {
      const selectedRows = yield select((state) => state.carousel.selectedRows);
    ...
    *export({ payload }, { call, select }) {
      ...
      const { queryParams, columns, fields } = yield select((state) => state.carousel);
