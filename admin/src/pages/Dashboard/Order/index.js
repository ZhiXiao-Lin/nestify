...
export default class extends PureComponent {
  ...
  getStatus = row => {
    if (row.status === 3) return <p>已失败</p>;
    if (row.statusOther === 4) return <p>强平中</p>;
    if (row.statusOther === 3 && row.status === 4) return <p>已撤单</p>;
    if (row.statusOther === 5 && row.status === 4) return <p>已强平</p>;

    if (row.orderType === 1) {
      if (row.status === 4) return <p>已成交</p>;
      if (row.statusOther === 2 && row.status === 2) return <p>撤单中</p>;
    } else {
      if (row.status === 4) return <p>已结算</p>;
      if (row.statusOther === 2 && row.status === 2) return <p>撤单中</p>;
    }
            ...
            <StandardTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  render(rowData) {
                    return rowData === 1 ? (
                      <Badge status="success" text="买" />
                    ) : (
                      <Badge status="error" text="卖" />
                    );
                  },
                },
                {
