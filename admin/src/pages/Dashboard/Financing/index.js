...
const CreateForm = Form.create()((props) => {
	...
	return (
		<Modal destroyOnClose title="新建规则" visible={modalVisible} onOk={okHandle} onCancel={() => handleModalVisible()}>
...
@connect(({ financing, loading }) => ({
	data: financing.data,
	loading: loading.models.financing
}))
@Form.create()
						...
						<StandardTable
							...
							columns={[
								...
								},
								{
									...
									dataIndex: 'type',
									render(val, rowData) {
										return rowData.dayOrMonth === EarnestType.DAY ? '按交易日' : '按月';
									}
								},
								{
									title: '用户名',
...

								{
									...
									render(val, rowData) {
										return `${rowData.interestValue.toFixed(2)} / ${rowData.dayOrMonth ===
										EarnestType.DAY
											? '交易日'
											: '月'}`;
									}
								...
								},
								{
									...
									dataIndex: 'repayment',
									render(val, rowData) {
										return (rowData.borrowAll - rowData.borrow).toFixed(2);
									}
								},
								// {
								// 	title: '状态',
							...
							expandedRowRender={(record) => (
								<p>
									支付凭证:{' '}
									<img style={{ maxWidth: 400 }} src={config.UPLOAD_PATH + record.certificate} />
								</p>
							)}
